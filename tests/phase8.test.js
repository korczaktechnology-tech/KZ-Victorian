...
test("Fase 8: seleção usa Spatial Partition e emite evento",()=>{const w=createSimulationWorld(null,32,32,32);w.bootstrap();const r=w.selectAt(0,0,1);assert.equal(r.ok,true);assert.equal(r.selection.entityId,w.playerId);assert.equal(w.events.at(-1).type,"selectionChanged");});
