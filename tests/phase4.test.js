...
test("Fase 4: um passo executa os oito sistemas e publica snapshot transferível",()=>{const{simulation}=createSimulation();const r=simulation.step();assert.equal(r.tick,1);assert.deepEqual(r.systemTrace,["Population","Pathfinding","Movement","Needs","Economy","Production","Construction","Logistics"]);assert.equal(workerMessages.at(-1)?.type,"snapshot");assert.ok(workerMessages.at(-1)?.payload.positions instanceof ArrayBuffer);});
