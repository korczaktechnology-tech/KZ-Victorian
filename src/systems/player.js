...
export function createPlayerController(world){
 ...
 return{
  setInput(next={}){input={forward:Number(next.forward)||0,right:Number(next.right)||0,sprint:Boolean(next.sprint),jump:Boolean(next.jump)};},
  update(dt=1/30){
   ...
   if(input.jump&&grounded){verticalVelocity=JUMP_SPEED;grounded=false;}
   ...
   const nx=input.right/len,ny=input.forward/len,yaw=world.playerFacing,s=Math.sin(yaw),c=Math.cos(yaw);
