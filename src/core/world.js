...
export function createSimulationWorld(memory=null,capacity=MEMORY_CAPACITIES.entities,width=WORLD.DEFAULT_WIDTH,height=WORLD.DEFAULT_HEIGHT){
 ...
 spawn(type,x=0,y=0,z=0){const id=spawnInitialEntity(world,type,x,y,z);world.syncEntityToMap(id);return id;},emit(type,payload=null){world.events.push({type,payload});},
 syncEntityToMap(id){if(!entities.has(id))return false;const p=entities.get(id,"Position");return p?map.syncEntity(id,entities.type(id),p):false;},rebuildSpatial(){spatial.rebuild(entities);},
 ...
 requestConstruction(type,x,y,z=0){return requestConstruction(world,type,x,y,z);},createLogisticsTask(payload){return createLogisticsTask(world,payload);},createLogisticsRequest(payload){return createLogisticsRequest(world,payload);},
 ...
 selectAt(x,y,radius=1.5){let selected=null;world.spatial.queryRadius(x,y,radius,id=>entities.get(id,"Position"),(id,pos)=>{if(!selected)selected={entityId:id,entityType:entities.type(id),x:pos[0],y:pos[1],z:pos[2]??0};});world.emit("selectionChanged",selected);return{ok:true,selection:selected};}
 };
 world.playerController=createPlayerController(world);world.collisionSystem=createCollisionSystem(world);world.resolvePlayerCollision=(id,sx,sy,tx,ty,r)=>world.collisionSystem.resolvePlayer(id,sx,sy,tx,ty,r);
 world.bootstrap=()=>{if(entities.size!==0)return;world.playerId=world.spawn(EntityType.HABITANT,0,0,0);world.spawn(EntityType.HOUSE,0,4,0);world.spawn(EntityType.WAREHOUSE,4,0,0);world.spawn(EntityType.SAWMILL,8,0,0);world.spawn(EntityType.ROAD,2,2,0);world.spawn(EntityType.HABITANT,1,0,0);world.spawn(EntityType.TREE,6,0,0);world.spawn(EntityType.RESOURCE,10,0,0);world.spawn(EntityType.ANIMAL,12,0,0);world.rebuildSpatial();world.setFlowField("default",[{x:2,y:0}]);world.metrics.resources=world.resourceTotals();};
 return world;
}
