...
import{createPlayerRenderer}from"./player-renderer.js";
export function createRenderer(canvas,o={}){
 ...
 const gpu=createGPUProfiler(gl),camera=o.camera??new Camera(),terrain=createTerrainMesh(gl,256,256);
 const terrainProgram=createProgram(gl,TERRAIN_VERTEX_SHADER,TERRAIN_FRAGMENT_SHADER),terrainVao=gl.createVertexArray();if(!terrainVao)throw new Error("WebLords: falha ao criar VAO do terreno.");
 const tp=gl.getAttribLocation(terrainProgram,"a_position"),tn=gl.getAttribLocation(terrainProgram,"a_normal"),tmat=gl.getUniformLocation(terrainProgram,"u_viewProjection");if(tp<0||tn<0||!tmat)throw new Error("WebLords: atributos/uniforme do terreno não encontrados.");
 ...
 const playerRenderer=createPlayerRenderer(gl,createProgram,sampleTerrainHeight);let drawCalls=0,visibleInstances=0,culledInstances=0,lastMetrics={drawCalls:0,visibleInstances:0,culledInstances:0,gpuMs:0,gpuTimerSupported:gpu.supported};
 ...
 function render(time,snapshot={}){
  ...
  if(snapshot.player)camera.setPosition(snapshot.player.position[0],snapshot.player.position[1]);
  ...
  for(let i=0;i<culled.count;i++){const x=culled.data[i*3],z=culled.data[i*3+1],height=sampleTerrainHeight(x,z,terrain.size);data[i*4]=x;data[i*4+1]=height+1.1;data[i*4+2]=z;data[i*4+3]=1.15;}instanceBuffer.upload(culled.count);
  ...
  playerRenderer.draw(vp,snapshot.player);visibleInstances=culled.count;culledInstances=culled.culled;drawCalls=1+(culled.count?1:0)+6;gpu.end(gpuQuery);const gpuMs=gpu.resolve();profiler.endFrame(frame);lastMetrics={drawCalls,visibleInstances,culledInstances,gpuMs,gpuTimerSupported:gpu.supported};
 }
